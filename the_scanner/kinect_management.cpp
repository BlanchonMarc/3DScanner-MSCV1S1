...
Kinect_Management::Kinect_Management()
{
...
//Boost lock mutex, this is members of the class boost which is a tool box //
//A mutex is an object that can be locked to take care of security
// The lock is the locker or the security guard assigned to this mutex.
...
pcl::PointCloud<pcl::PointXYZ>::Ptr pcl::Kinect2Grabber::convertDepthToPointXYZ( UINT16* depthBuffer )
{
    ...
    //static_cast to give statically the type and convert if necessary
...
//return type is a compressed way of returning a cloud by using the pointer assigned to it
